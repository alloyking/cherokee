<div class="relative flex min-h-screen flex-col items-center justify-center p-6 lg:p-8">
    ...
    <div class="fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_center,var(--color-accent)_0%,transparent_50%)] opacity-20"></div>
    ...
    <div class="w-full max-w-[400px] z-10 transition-opacity opacity-100 duration-750 starting:opacity-0 starting:translate-y-4">
        ...
        <div class="text-center mb-8">
            <a href="{{ url('/') }}" class="inline-block focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] focus:ring-offset-2 focus:ring-offset-zinc-950 rounded" aria-label="deceptio.ai – Home">
                <img src="{{ asset('images/logo-header.png') }}" alt="deceptio.ai" class="mx-auto h-12 w-auto sm:h-14" width="240" height="48">
            </a>
        </div>

        <main class="rounded-3xl border border-white/10 bg-zinc-900/40 p-8 shadow-2xl shadow-black/50 backdrop-blur-xl w-full">

            <div class="mb-6 space-y-1 text-center">
                <p class="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--color-accent)]">Cherokee</p>
            ...
            <form wire:submit="login" class="space-y-4">
                ...
                <button
                    ...
                    class="w-full rounded-xl bg-[var(--color-accent)] px-6 py-4 text-sm font-medium text-[var(--color-accent-foreground)] transition hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2"
                >
